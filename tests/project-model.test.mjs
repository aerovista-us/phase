...
const makeState=()=>({bpm:98,viewDuration:180,pxPerSecond:12,snapMode:'16bar',tracks:[{label:'TRACK A',name:'a.wav',file:{name:'a.wav'},buffer:{},sourceBpm:97.5,pitch:0,timelineOffset:1.25,gridMode:'detected',alignMarker:8,gainDb:-2.5,mute:false,solo:true,analysis:{key:'Am',keyRoot:9,keyMode:'minor',keyConfidence:.8},markers:[{beat:0,sourceTime:0,targetTime:0,downbeat:true,locked:true,confidence:1},{beat:1,sourceTime:.6,targetTime:.62,downbeat:false,locked:false,confidence:.5}]}]});

test('Phase project snapshots preserve arrangement and mix metadata',()=>{const s=makeState(),p=snapshotProject(s);assert.equal(p.version,5);assert.equal(p.snapMode,'16bar');assert.equal(p.tracks[0].fileName,'a.wav');assert.equal(p.tracks[0].alignMarker,8);assert.equal(p.tracks[0].gainDb,-2.5);assert.equal(p.tracks[0].analysis.key,'Am');assert.equal(p.tracks[0].markers[1].targetTime,.62)});

test('project snapshot applies to loaded track without replacing audio object',()=>{const source=snapshotProject(makeState()),target=makeState(),audio=target.tracks[0].buffer;target.bpm=120;target.tracks[0].timelineOffset=0;target.tracks[0].markers[1].targetTime=.6;applyProjectSnapshot(target,source,{loadedOnly:true});assert.equal(target.bpm,98);assert.equal(target.tracks[0].timelineOffset,1.25);assert.equal(target.tracks[0].markers[1].targetTime,.62);assert.equal(target.tracks[0].buffer,audio)});
