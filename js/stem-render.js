...
export async function renderTrackSources(track,onProgress,options={}){
  ...
  if(!trackHasDspEdits(track)){for(const stem of stems){stem.renderedBuffer=null;stem.renderedSignature=null;stem.renderStats={quality:'direct',processedGrains:0,totalGrains:0,workRatio:0,dirtyRanges:[]}}onProgress?.(1);return stems.map(s=>s.buffer)}
...
export function clearInactiveRenderState(track){
  ...
  if(usingStemSources(track)){for(const stem of loadedStems(track))if(!trackHasDspEdits(track)){stem.renderedBuffer=null;stem.renderedSignature=null}return}
  if(!trackHasDspEdits(track)){track.renderedBuffer=null;track.renderedSignature=null}
}
export function sourceRenderStats(track){
  ensureStemState(track);if(usingStemSources(track))return loadedStems(track).map(s=>s.renderStats).filter(Boolean);return track.renderStats?[track.renderStats]:[];
}
