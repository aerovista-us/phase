...
export function activeAudioParts(track,{preferRendered=true,includeMuted=false}={}){
  ...
  if(track.useStems&&hasLoadedStems(track))return loadedStems(track).filter(s=>includeMuted||!s.mute).map(s=>({id:s.id,label:s.label,buffer:preferRendered?(s.renderedBuffer||s.buffer):s.buffer,gain:stemGainLinear(s),stem:s})).filter(x=>x.buffer);
...
export function stemProjectMetadata(track){ensureStemState(track);return{useStems:!!track.useStems,stems:Object.fromEntries(STEM_TYPES.map(id=>{const s=track.stems[id];return[id,{fileName:s.fileName||null,gainDb:num(s.gainDb,0),mute:!!s.mute}]}))}}
export function applyStemProjectMetadata(track,meta){ensureStemState(track);if(!meta)return track;track.useStems=!!meta.useStems;for(const id of STEM_TYPES){const src=meta.stems?.[id];if(!src)continue;const s=track.stems[id];s.fileName=src.fileName||s.fileName||null;s.gainDb=clamp(num(src.gainDb,s.gainDb),-24,12);s.mute=!!src.mute}if(track.useStems&&!hasLoadedStems(track))track.useStems=false;return track}
