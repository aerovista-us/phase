...
export function applyStemProjectMetadata(track,meta){ensureStemState(track);if(!meta)return track;track.useStems=!!meta.useStems;for(const id of STEM_TYPES){const src=meta.stems?.[id];if(!src)continue;const s=track.stems[id];s.fileName=src.fileName||s.fileName||null;if(!(s.buffer&&s.identity))s.identity=normalizeIdentity(src.identity)||s.identity||null;s.gainDb=clamp(num(src.gainDb,s.gainDb),-24,12);s.mute=!!src.mute;s.solo=!!src.solo}if(track.useStems&&!hasLoadedStems(track))track.useStems=false;return track}
