...
export function clearStem(track,id){if(!STEM_TYPES.includes(id))return;ensureStemState(track);track.stems[id]={id,label:id.toUpperCase(),buffer:null,renderedBuffer:null,renderedSignature:null,fileName:null,gainDb:0,mute:false,solo:false,renderStats:null,durationWarning:false}}
...
export function stemProjectMetadata(track){ensureStemState(track);return{useStems:!!track.useStems,stems:Object.fromEntries(STEM_TYPES.map(id=>{const s=track.stems[id];return[id,{fileName:s.fileName||null,gainDb:num(s.gainDb,0),mute:!!s.mute,solo:!!s.solo}]}))}}
