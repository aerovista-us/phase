...
function normalizeItem(item){return item?.buffer?{gain:1,group:null,sourceIn:0,sourceOut:null,fadeInStart:null,fadeInEnd:null,fadeOutStart:null,fadeOutEnd:null,...item}:{buffer:item,offset:0,gain:1,group:null,sourceIn:0,sourceOut:null,fadeInStart:null,fadeInEnd:null,fadeOutStart:null,fadeOutEnd:null}}
...
export function mixNormalizationCount(items){const list=(items||[]).map(normalizeItem),keys=list.map((x,i)=>x.group==null?`item:${i}`:`group:${x.group}`);return Math.max(1,new Set(keys).size)}
...
export async function renderMix(items, sampleRate = 44100) {
  ...
  const perTrack=1/mixNormalizationCount(active.map(x=>x.item));
  for(const {item,w} of active){const source=off.createBufferSource(),gain=off.createGain(),base=perTrack*Math.max(0,Number.isFinite(item.gain)?item.gain:1),p0=w.delay,p1=w.delay+w.duration;source.buffer=item.buffer;applyGainEnvelope(gain.gain,base,item,p0,p1,w.delay);source.connect(gain).connect(master);source.start(w.delay,w.sourceOffset,w.duration)}
...
export async function renderMixRegion(items,start,end,sampleRate=44100){
  ...
  const active=playable.map(item=>({item,w:mixRegionWindow(item.offset||0,item.buffer.duration,r0,r1,item.sourceIn||0,item.sourceOut)})).filter(x=>x.w),perTrack=1/mixNormalizationCount(active.map(x=>x.item));
