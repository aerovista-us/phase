...
import{createProjectPackage,packageSummary}from'./project-package.js';
...
function downloadable(data,fileName='phase-project.json'){const blob=new Blob([JSON.stringify(data,null,2)],{type:'application/json'}),url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download=fileName;a.click();setTimeout(()=>URL.revokeObjectURL(url),1200)}
function remember(data){try{localStorage.setItem(STORE,JSON.stringify(data));lastAuto=editableFingerprint(state)}catch{}refreshRestoreButton()}
function saveProject(){const data=snapshotProject(state);downloadable(data);remember(data);$('#engineState').textContent='PHASE PROJECT SAVED · SESSION REMEMBERED'}
function packageName(){const names=state.tracks.map(t=>t.file?.name||t.fileName).filter(Boolean).map(n=>String(n).replace(/\.[^.]+$/,''));return names.length?names.join(' + '):'Phase Project'}
function savePackage(){const data=createProjectPackage(state,{name:packageName()}),summary=packageSummary(data);downloadable(data,'phase-project.phase.json');remember(data);$('#engineState').textContent=`PHASE PACKAGE SAVED · ${summary.assetCount} ASSET${summary.assetCount===1?'':'S'} MANIFESTED`}
...
async function loadProjectFile(file){const data=validateProject(JSON.parse(await file.text())),summary=packageSummary(data);try{localStorage.setItem(STORE,JSON.stringify(data))}catch{}applyLoadedFrom(data);refreshRestoreButton();if(summary.packaged&&!state.tracks.some(t=>t.buffer))$('#engineState').textContent=`PACKAGE LOADED · ${summary.assetCount} ASSET${summary.assetCount===1?'':'S'} EXPECTED · USE RELINK FILES`}
...
function installControls(){
  if(!$('#savePackage')){const b=document.createElement('button');b.className='btn';b.id='savePackage';b.textContent='SAVE PACKAGE';b.title='Save a Phase project package with an explicit source/stem asset manifest (audio bytes are not embedded)';$('#saveMap').after(b);b.onclick=savePackage}
