...
export function trackDiagnostics(track){
  const buffer=track?.buffer||null,analysis=track?.analysis||null,stems=track?.stems&&typeof track.stems==='object'?Object.values(track.stems):[],loadedStems=stems.filter(s=>s?.buffer),warnings=[];
  ...
  return{
    ...
    duration:buffer?round(buffer.duration):round(track?.duration),sampleRate:buffer?num(buffer.sampleRate):null,channels:buffer?num(buffer.numberOfChannels):null,
...
export function projectDiagnostics(state,environment={}){
  const tracks=(state?.tracks||[]).map(trackDiagnostics),warnings=[];
  ...
  return{
    ...
    environment:{...environment},project:{bpm:round(state?.bpm||120,2),meter:state?.meter||'4/4',dirty:!!state?.dirty,rendering:!!state?.rendering,playing:!!state?.playing,viewDuration:round(state?.viewDuration||0),zoomPxPerSecond:round(state?.pxPerSecond||0)},
