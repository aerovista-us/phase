...
async function exportLoop(){
  ...
  const tracks=eligible(),items=tracks.map(t=>({buffer:t.renderedBuffer||t.buffer,offset:t.renderedOffset||0,gain:dbToGain(t.gainDb)}));if(!items.length)return alert('No audible tracks to export.');
  const btn=$('#exportLoop');btn.disabled=true;$('#engineState').textContent=`EXPORTING ${state.loopBars} BAR LOOP…`;
  try{const mix=await renderMixRegion(items,state.loopStart,state.loopEnd,44100);downloadWav(mix,`phase-loop-${state.loopBars}bars.wav`);$('#engineState').textContent=`LOOP WAV EXPORTED · ${state.loopBars} BARS`}
  catch(err){console.error(err);$('#engineState').textContent='LOOP EXPORT FAILED';alert('Loop export failed: '+(err.message||err))}
  finally{btn.disabled=false}
}

function install(){if($('#exportLoop'))return;const btn=document.createElement('button');btn.className='mode';btn.id='exportLoop';btn.textContent='EXPORT LOOP';btn.title='Render the current audition loop to WAV using current mute, solo, and level settings';const size=$('#loopBars');if(size)size.after(btn);else $('.modebar').append(btn);btn.onclick=exportLoop}
